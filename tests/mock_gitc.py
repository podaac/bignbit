...
class GITC:
    ...
    def wait_for_messages(self, count: int = 1) -> List[Dict]:
    ...
    def process_messages(self):
        while True:
            messages = self._incoming_queue.receive_messages(WaitTimeSeconds=10)
            for message in messages:
                message_body = json.loads(message.body)
                self._received_messages.append(message_body)
